...
func TestRemovePeerTracking(t *testing.T) {
	id := randomID()
	primary := newPeer(log.Root(), &conn{node: newNode(id, "")}, nil)
...
func TestRemovePeerTrackingDeletesPrimaryPeer(t *testing.T) {
	id := randomID()
	primary := &Peer{rw: &conn{node: newNode(id, "")}}
	peers := map[enode.ID]*Peer{id: primary}

	remaining := removePeerTracking(peers, peerDrop{Peer: primary}, 1)
	if remaining != 0 {
		t.Fatalf("unexpected connection count: got %d want 0", remaining)
	}
	if peers[id] != nil {
