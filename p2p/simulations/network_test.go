...
func TestNetworkSimulation(t *testing.T) {
	...
	check := func(ctx context.Context, id enode.ID) (bool, error) {
		...
		// NOTE: Upstream geth expects exactly 2 here (one logical neighbor on each side).
		// In this fork, the p2p layer may temporarily or permanently keep a paired
		// second physical connection for the same remote NodeID.
		// test_peerCount in this test service counts protocol runs (physical conns),
		// not deduplicated remote NodeIDs, so a ring node can observe 2..4.
