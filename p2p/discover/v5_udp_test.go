...
import (
	...
	"github.com/XinFinOrg/XDPoSChain/p2p/discover/v5wire"
...
func TestUDPv5_lookupE2E(t *testing.T) {
	...
	// Do lookups until the target is discovered.
	...
	for {
		...
		if time.Now().After(deadline) {
			t.Fatalf("lookup failed to discover target %v, got %d nodes", target.ID(), len(results))
		}
		time.Sleep(50 * time.Millisecond)
	}

	if !sort.SliceIsSorted(results, func(i, j int) bool {
		return enode.DistCmp(target.ID(), results[i].ID(), results[j].ID()) < 0
	}) {
		t.Fatalf("lookup returned nodes out of distance order")
	}
}
