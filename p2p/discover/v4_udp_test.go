...
func TestUDP_pingPacketRejected(t *testing.T) {
	...
	const (
		testMACSize  = 32
		testHeadSize = testMACSize + crypto.SignatureLength
	)

	enc, _, err := v4wire.Encode(test.remotekey, &v4wire.Ping{From: testRemote, To: testLocalAnnounced, Version: 4, Expiration: futureExp})
	if err != nil {
		t.Fatalf("encode failed: %v", err)
	}
	enc[testHeadSize] = 1
	sig, err := crypto.Sign(crypto.Keccak256(enc[testHeadSize:]), test.remotekey)
	if err != nil {
		t.Fatalf("resign failed: %v", err)
	}
	copy(enc[testMACSize:], sig)
	hash := crypto.Keccak256(enc[testMACSize:])
	copy(enc, hash)
