...
var testChainConfig = func() *params.ChainConfig {
	...
	if config.XDPoS != nil {
		...
		if xdpos.RewardCheckpoint == 0 {
			xdpos.RewardCheckpoint = 900
		}
		config.XDPoS = &xdpos
	}
...
var xdposTestSignature = func() []byte {
	sig, err := crypto.Sign(common.Hash{}.Bytes(), testKey)
	if err != nil {
		panic(fmt.Errorf("failed to sign XDPoS test header: %w", err))
	}
	return sig
}()
...
func newTestBackend(config *node.Config) (*node.Node, []*types.Block, error) {
	...
	if config.DataDir == "" {
		dataDir, err := os.MkdirTemp("", "xdpos-ethclient-")
		if err != nil {
			return nil, nil, fmt.Errorf("can't create temp datadir: %v", err)
		}
	...
	// Create XDCx and lending services required by eth.New
	xdcx := XDCx.New(n, &XDCx.Config{DataDir: filepath.Join(config.DataDir, "XDCx")})
	lending := XDCxlending.New(n, xdcx)
	// Create Ethereum Service
	ecfg := &ethconfig.Config{Genesis: genesis, RPCGasCap: 1000000}
	ethservice, err := eth.New(n, ecfg, xdcx, lending)
	if err != nil {
...
func generateTestChain() []*types.Block {
	generate := func(i int, g *core.BlockGen) {
		...
		copy(extra[len(extra)-utils.ExtraSeal:], xdposTestSignature)
		g.SetExtra(extra)
		if i == 1 {
...
func testHeader(t *testing.T, chain []*types.Block, client *rpc.Client) {
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if tt.want == nil {
				if got != nil {
					t.Fatalf("HeaderByNumber(%v) got = %v, want nil", tt.block, got)
				}
				return
			}
			if got == nil {
				t.Fatalf("HeaderByNumber(%v) got = nil, want %v", tt.block, tt.want)
			}
...
func testBalanceAt(t *testing.T, client *rpc.Client) {
	...
	tests := map[string]struct {
		...
		"future_block": {
			...
			block:   big.NewInt(1000000000),
			want:    big.NewInt(0),
			wantErr: errors.New("header for number not found"),
		},
	}
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
...
func testTransactionInBlock(t *testing.T, client *rpc.Client) {
	...
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != ethereum.ErrNotFound {
		t.Fatal("error should be ethereum.ErrNotFound")
	}
...
func testChainID(t *testing.T, client *rpc.Client) {
	...
	if id == nil || id.Cmp(genesis.Config.ChainID) != 0 {
...
func testStatusFunctions(t *testing.T, client *rpc.Client) {
	...
	if networkID.Cmp(genesis.Config.ChainID) != 0 {
	...
	if gasPrice.Cmp(big.NewInt(params.InitialBaseFee)) != 0 {
	...
	if history.OldestBlock == nil || history.OldestBlock.Cmp(big.NewInt(2)) != 0 {
		t.Fatalf("unexpected oldest block: %v", history.OldestBlock)
	}
	if len(history.Reward) != 1 || len(history.Reward[0]) != 2 {
		t.Fatalf("unexpected reward history: %v", history.Reward)
	}
	if len(history.BaseFee) != 2 || history.BaseFee[0].Sign() <= 0 || history.BaseFee[1].Sign() <= 0 {
		t.Fatalf("unexpected base fee history: %v", history.BaseFee)
	}
	if len(history.GasUsedRatio) != 1 {
		t.Fatalf("unexpected gas used ratio: %v", history.GasUsedRatio)
	}
}
...
func testAtFunctions(t *testing.T, client *rpc.Client) {
	...
	baseNonce, err := ec.NonceAt(context.Background(), testAddr, nil)
	if err != nil {
	...
	for {
		...
		pendingNonce, err := ec.PendingNonceAt(context.Background(), testAddr)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if pending > 0 || pendingNonce == baseNonce+1 {
			break
		}
		if time.Now().After(deadline) {
			t.Fatalf("pending transaction not observed")
		}
	...
	if penBalance.Cmp(balance) > 0 {
		t.Fatalf("unexpected pending balance: %v %v", balance, penBalance)
	}
	// NonceAt
	...
	// Verify that sender address of transaction from latest block is saved in cache.
	latestBlock, err := ec.BlockByNumber(context.Background(), nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(latestBlock.Transactions()) == 0 {
		t.Fatal("latest block does not contain transactions")
	}
	// No additional RPC should be required, ensure the server is not asked by
	// canceling the context.
	sender, err := ec.TransactionSender(newCanceledContext(), latestBlock.Transactions()[0], latestBlock.Hash(), 0)
	if err != nil {
