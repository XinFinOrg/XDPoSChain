	...
	if n != nil {
		...
		for uint64(fp.nodeCnt)+head.Number-n.number > maxNodeCount && fp.root != nil {
			...
			for i, nn := range fp.root.children {
				if core.ReadCanonicalHash(f.pm.chainDb, nn.number) == nn.hash {
...
func (f *lightFetcher) peerHasBlock(p *peer, hash common.Hash, number uint64) bool {
	...
	return core.ReadCanonicalHash(f.pm.chainDb, fp.root.number) == fp.root.hash && core.ReadCanonicalHash(f.pm.chainDb, number) == hash
}
