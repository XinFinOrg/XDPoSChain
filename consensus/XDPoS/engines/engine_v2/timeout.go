...
func (x *XDPoS_v2) sendTimeout(chain consensus.ChainReader) error {
	...
	if isEpochSwitch {
		...
		var ok bool
		gapNumber, ok = x.config.GapBlockNumber(currentNumber)
		if !ok {
			return gapPathError("[sendTimeout]", currentNumber, x.config)
		}
		...
		if !ok {
			return gapPathError("[sendTimeout]", epochSwitchNumber, x.config)
		}
		log.Debug("[sendTimeout] non-epoch-switch block found its epoch block and calculated the gapNumber", "epochSwitchInfo.EpochSwitchBlockInfo.Number", epochSwitchNumber, "gapNumber", gapNumber)
	}
