...
type SnapshotV2Subnet struct {
	...
	NextEpochCandidates []common.Address `json:"masterNodes"` // Set of authorized master nodes at this moment for next epoch
...
func newSnapshot(number uint64, hash common.Hash, candidates []common.Address, penalties []common.Address) *SnapshotV2Subnet {
	snap := &SnapshotV2Subnet{
		Number:              number,
		Hash:                hash,
...
func loadSnapshot(db ethdb.Database, hash common.Hash) (*SnapshotV2Subnet, error) {
	blob, err := db.Get(append([]byte("XDPoS-V2-"), hash[:]...))
	if err != nil {
		return nil, err
	}
	snap := new(SnapshotV2Subnet)
...
func storeSnapshot(s *SnapshotV2Subnet, db ethdb.Database) error {
...
// func (s *SnapshotV2Subnet) GetMappedMasterNodes() map[common.Address]struct{} {
func (s *SnapshotV2Subnet) GetMappedCandidates() map[common.Address]struct{} {
...
func (s *SnapshotV2Subnet) IsMasterNodes(address common.Address) bool {
	for _, n := range s.NextEpochCandidates {
...
func (x *XDPoS_v2) getSnapshot(chain consensus.ChainReader, number uint64, isGapNumber bool) (*SnapshotV2Subnet, error) {
	...
	if snap, ok := x.snapshots.Get(gapBlockHash); ok {
...
// snapshot retrieves the authorization snapshot at a given point in time.
func (x *XDPoS_v2) getSnapshotByHash(gapBlockHash common.Hash) (*SnapshotV2Subnet, error) {
	// If an in-memory SnapshotV2 was found, use that
	if snap, ok := x.snapshots.Get(gapBlockHash); ok {
