...
func TestHookGetSignersFromContractUsesRequestedBlockState(t *testing.T) {
	...
	setCandidateCap(t, askedState, subject, new(big.Int).Add(highest, big.NewInt(1)))
	...
	require.NoError(t, err)
	headState.SetState(common.MasternodeVotingSMCBinary, candidateCapSlot(subject), common.Hash{})
...
func setCandidateCap(t *testing.T, statedb *state.StateDB, candidate common.Address, cap *big.Int) {
	t.Helper()
