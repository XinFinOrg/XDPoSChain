...
// Shared global denylist for all networks, keyed by the version each cohort of
// addresses belongs to. A version fixes the height from which its addresses are
// denied: version 1 activates at ChainConfig.DenylistBlock, and every later version
// at ChainConfig.DenylistActivations[version].
//
// To add addresses, add a NEW version key. Never add to an existing version and
// never move an address between versions: either changes the height an address
// becomes denied. Moved to a later version it stops being denied until that version
// activates, unfreezing it in the meantime; moved to an earlier version it becomes
// denied over history where it was allowed, so blocks that were valid when sealed
// now fail and replay breaks for any node syncing from genesis.
//
// The version is the map key so it is stated exactly once. A repeated key does not
// compile, and a map has no order, so a version cannot be renumbered by reordering.
var denylistVersions = map[uint8][]Address{
	1: {
	...
	2: {
		...
		HexToAddress("0xcc4a939c8289bfefb9723ef68b9cd14187f2097a"),
	},
}

func init() {
	validateDenylistVersions(denylistVersions)
}

// validateDenylistVersions rejects any literal that would deny the wrong addresses
// from the wrong height. It builds nothing; DenylistVersion reads denylistVersions
// directly so the literal stays the single representation of the denylist.
//
// The duplicate check cannot be left to the compiler: Go only rejects duplicate map
// keys when they are compile-time constants, and HexToAddress is a function call.
// In an address-keyed literal the same address written under two versions would
// compile and the last one would silently win, rescheduling it with no trace.
// Keying by version puts the addresses in slices, which keep every entry, so the
// repeat is visible here.
func validateDenylistVersions(versions map[uint8][]Address) {
	seen := make(map[Address]uint8)
	// Walk 1..len so a missing version is caught. A gap means some cohort was
	// tagged with a version nothing schedules, leaving it never denied.
	for version := uint8(1); version <= uint8(len(versions)); version++ {
		addresses, scheduled := versions[version]
		if !scheduled {
			panic(fmt.Sprintf("denylist versions must be contiguous from 1: version %d is missing", version))
		}
		if len(addresses) == 0 {
			panic(fmt.Sprintf("denylist version %d has no addresses", version))
		}
		for _, address := range addresses {
			if existing, listed := seen[address]; listed {
				panic(fmt.Sprintf("denylist address %s is listed in both version %d and version %d", address.Hex(), existing, version))
			}
			seen[address] = version
		}
	}
	if len(seen) == 0 {
		panic("denylist is empty")
	}
}
...
// DenylistVersion returns the denylist version address belongs to, and
// whether it is listed at all. Callers must gate the result on the version's
// activation height; core.IsDeniedSender and core.IsDeniedReceiver do this.
//
// denylistVersions is scanned directly rather than through a prebuilt address
// index, so the literal remains the only representation of the denylist. Each
// address belongs to exactly one version, which init enforces, so the first match
// is the answer.
func DenylistVersion(address *Address) (uint8, bool) {
	if address == nil {
		return 0, false
	}
