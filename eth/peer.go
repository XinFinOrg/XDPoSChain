...
func (ps *peerSet) PeersWithoutVote(hash common.Hash) []*peer {
	...
	list := make([]*peer, 0, len(ps.peers))
	for _, p := range ps.peers {
		if p.version >= xdpos2 && !p.knownVote.Contains(hash) {
			list = append(list, p)
		}
	}
	return list
...
func (ps *peerSet) PeersWithoutTimeout(hash common.Hash) []*peer {
	...
	list := make([]*peer, 0, len(ps.peers))
	for _, p := range ps.peers {
		if p.version >= xdpos2 && !p.knownTimeout.Contains(hash) {
			list = append(list, p)
		}
	}
	return list
...
func (ps *peerSet) PeersWithoutSyncInfo(hash common.Hash) []*peer {
	...
	list := make([]*peer, 0, len(ps.peers))
	for _, p := range ps.peers {
		if p.version >= xdpos2 && !p.knownSyncInfo.Contains(hash) {
			list = append(list, p)
		}
	}
	return list
