...
func TestFindAncestorSpanRefinesBelowLowestSample(t *testing.T) {
	...
	// the local head.
	from, max := calculateRequestSpan(uint64(chainLen-1), headHeight)
	if from != int64(headHeight-2) || max != headHeight {
		t.Fatalf("unexpected span window: from %d max %d", from, max)
	}
...
// TestFindAncestorSpanRemoteHeadAtMaxUint64 pins the overflow boundary of
// the gap refinement's remote-head clamp: the remote head number reaches
// findAncestor through fetchHeight, which validates the response's shape
// but not its height, so a misbehaving peer can advertise a head at
// math.MaxUint64. The refinement's exclusive remote bound must not wrap
// remoteHeight+1 to zero: a zero end silently skips the binary search and
// anchors the sync one block lower than the search actually resolved.
func TestFindAncestorSpanRemoteHeadAtMaxUint64(t *testing.T) {
	t.Parallel()
	...
	// Peer on a heavy fork sharing blocks 0..forkPoint-1 with the base chain:
	// the span window {head-2, head} then rejects the head sample and hits its
	// lower sample, seeding the gap refinement the overflow must not skip.
	fork := testChainBase.shorten(forkPoint).makeFork(chainLen-forkPoint, true, 7)
	if err := tester.newPeer("peer", xdc165, fork); err != nil {
	...
	atomic.StoreUint32(&tester.downloader.mode, uint32(FullSync))

	// DeliverHeaders drops replies unless a sync is marked active, so arm the
	// cancellation channel the way Synchronise would before driving the search
	// directly.
	tester.downloader.cancelLock.Lock()
	tester.downloader.cancelCh = make(chan struct{})
	tester.downloader.cancelLock.Unlock()

	// fetchHeight never validates the advertised height, so hand the search
	// a head at the wrap boundary directly.
	remoteHead := &types.Header{Number: new(big.Int).SetUint64(math.MaxUint64)}
	ancestor, err := tester.downloader.findAncestor(peer, remoteHead)
	if err != nil {
		t.Fatalf("failed to find ancestor: %v", err)
	}
	if want := uint64(headHeight - 1); ancestor != want {
		t.Fatalf("gap refinement skipped: ancestor %d want %d", ancestor, want)
	}
}
...
func TestFindAncestorBinarySearchIgnoresBlocksAboveHead(t *testing.T) {
	...
	for i := 0; i < spanSampleCount; i++ {
		num := int(from) + i*(spanSampleSkip+1)
...
func TestRemoteHeaderRequestSpan(t *testing.T) {
	...
	reqs := func(from int) []int {
		var r []int
		num := from
		for len(r) < spanSampleCount {
			r = append(r, num)
			num += spanSampleSkip + 1
		}
		return r
	}
	for i, tt := range testCases {
		from, max := calculateRequestSpan(tt.remoteHeight, tt.localHeight)
		data := reqs(int(from))

		// Cross-check the returned max against the request window itself:
		// max must stay in lockstep with the issued request, since findAncestor
